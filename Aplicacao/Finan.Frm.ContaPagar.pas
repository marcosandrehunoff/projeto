...
uses
  ...
  Fwk.Frm.Padrao.Pesquisa, Finan.Frm.Entidade, Finan.Frm.Categoria, Fwk.Dm.Padrao.Database;
...
procedure TFrmContaPagar.Button2Click(Sender: TObject);
begin
  inherited;
  with Pesquisar(DmCadastro.QryEntidade) do
  begin
    if Active then
    begin
      ...
      if (DmCadastro.QryContasPagar.State in [dsInsert, dsEdit]) then
        ...
        DmCadastro.QryContasPagarENT_ID.AsInteger := FieldByName('ENT_ID').AsInteger;
        DmCadastro.QryContasPagarENT_NOME.AsString := FieldByName('ENT_NOME').AsString;
      Close;
    end;
  end;
end;
